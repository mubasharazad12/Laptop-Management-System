...
def update_user_to_database(username , password):
    conn = connect_to_database()
    curosr = conn.cursor()
    curosr.execute("UPDATE users set password = %s where username = %s", (password , username))
    curosr.close()
    conn.commit()
    conn.close()
...
def varify_password(hashed_password, password):
    bcypt = Bcrypt()
    check = bcypt.check_password_hash(hashed_password.encode() , "1234")
    if check:
        return True
    return False